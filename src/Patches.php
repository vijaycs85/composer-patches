...
class Patches implements PluginInterface, EventSubscriberInterface
{
    ...
    protected function applyPatchWithGit($install_path, $patch_levels, $filename)
    {
        ...
        if (version_compare($this->getGitversion(), '2.14.0')  < 0) {
            return $this->patchWithDir($install_path, $patch_levels, $filename);
        }
     ...
     *   TRUE if patch was applied, FALSE otherwise.
     */
    protected function patchWithDir($install_path, $patch_levels, $filename)
    {
        $patched = false;
        foreach ($patch_levels as $patch_level) {
            $checked = $this->executeCommand(
    ...
    {
        $output = '';
        if ($this->executor->execute('git --version', $output) === 0) {
            return preg_match('/\d*\.\d*\.\d*/', $output, $matches);
        }
    }
}
